...
class OfficerCheckVehicleScreen extends StatefulWidget {
  ...
  @override
  State<OfficerCheckVehicleScreen> createState() => _OfficerCheckVehicleScreenState();
}
...
class _OfficerCheckVehicleScreenState extends State<OfficerCheckVehicleScreen> {
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    _isImageMode
                        ? 'أدخلي صورة واضحة للوحة المركبة، وسنحاول استخراج الرقم منها ثم متابعة الفحص تلقائياً.'
                  ...
                  if (_isImageMode)
                    OutlinedButton.icon(
                      onPressed: _isLoading ? null : _extractPlateFromImage,
                      icon: _isLoading
                          ? const SizedBox(
                              width: 18,
                              height: 18,
                      ...
                      label: Text(
                        _isLoading ? 'جارٍ استخراج اللوحة...' : 'اختيار صورة اللوحة',
                      ),
                    )
                  else
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'مثال: 24-381-15 أو 111222333',
                  ...
                  ElevatedButton.icon(
                    ...
                    label: Text(_isLoading ? 'جارٍ الفحص...' : 'فحص المركبة'),
                  ),
                ],
              ),
            ),
          ...
          if (_result != null)
            _ResultCard(
              result: _result!,
              onIssueFine: _result!.vehicleId == null
                  ? null
                  : () async {
                      final updated = await Navigator.of(context).push<bool>(
                        MaterialPageRoute(
                          builder: (_) => OfficerIssueFineScreen(result: _result!),
                        ),
                      );
                      ...
                      if (updated == true) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'تم إصدار المخالفة بنجاح. يمكنك الآن متابعة فحص مركبة أخرى.',
                            ),
                          ),
                        );
                      }
  ...
  Future<void> _checkVehicle() async {
    if (_isImageMode) {
      if (_plateController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('اختاري صورة اللوحة أو استخرجي الرقم أولاً.'),
          ),
        );
        return;
      }
    ...
    try {
      final result = await _inspectorService.checkVehicle(_plateController.text.trim());
      if (!mounted) return;
      setState(() {
        _result = result;
      });
  ...
  Future<void> _extractPlateFromImage() async {
    ...
    try {
      final plate = await _inspectorService.extractPlateFromImage(pickedFile.path);
      if (!mounted) return;
...
class _ResultCard extends StatelessWidget {
  ...
  String _statusDescription(OfficerVehicleCheckResult result) {
    ...
    if (result.isUnregistered) {
      return 'هذه المركبة غير موجودة في قاعدة البيانات الحالية للنظام.';
    }
      ...
      child: Ink(
        ...
        child: Column(
          children: [
            Icon(
              icon,
              color: isSelected ? const Color(0xFF0F766E) : const Color(0xFF5B6472),
            ),
